...
EasyAssess.directives["esFormGroup"] 
	...
	return {
		...
		template:  '<div class="es-form-group">' 
		          ...
		          +						'</tr>'
		          +						'<tr ng-repeat="row in esGroup.rows">'
		          +							'<td class="es-form-group-cell" ng-repeat="s_col in esGroup.specimens"><es-form-expect-option es-group-row="row" es-specimen-column="s_col"></es-form-expect-option></td>'
		          +						'</tr>'
		          +					'</table>'
		          +				'</td>'
		          +			'</tr>'
